...
def fix_ids():
    i = 0
    for cds_f in os.listdir('data/MycoCosm/cds'):
        cds_records = list()
        cds_path = os.path.join('data/MycoCosm/cds', cds_f)
    ...
    i = 0
    for prot_f in os.listdir('data/MycoCosm/proteomes'):
        prot_records = list()
        prot_path = os.path.join('data/MycoCosm/proteomes', prot_f)
